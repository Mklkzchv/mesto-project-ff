...
generateCard() {
  ...
  this._eventActiveLike = this._element.querySelector('.card__like-button');
  this._eventDeleteButton = this._element.querySelector('.card__delete-button');
  this._userElementCardImage.alt = 'Картинка ' + this._name;
  this._userElementCardImage.src = this._link;
  this._element.querySelector('.card__title').textContent = this._name;
  this._eventOpenImg = this._element.querySelector('.places__item');
...
_isSetLike() {
  if (this._likes.some((user) => { // проверяем элементы массива по условию переданной функции
    ...
    this._eventActiveLike.classList.add('card__like-button_is-active');
  }
}
//
...
// 
makeLike(item) {
  this._eventActiveLike.classList.toggle('card__like-button_is-active');
...
// 
  _setEventListeners() {
    ...
    this._eventActiveLike.addEventListener('click', () => {
        if (this._eventActiveLike.classList.contains('card__like-button_is-active')) {
